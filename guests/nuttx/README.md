...
export BAO_DEMOS_NUTTX_KCONFIG_VENV=$BAO_DEMOS_WRKDIR_BIN/nuttx-kconfig
export BAO_DEMOS_NUTTX_KCONFIG_BIN=$BAO_DEMOS_NUTTX_KCONFIG_VENV/bin
...
Before configuring NuttX, install the `kconfiglib` tools (`olddefconfig`,
`savedefconfig`, `setconfig`) expected by the CMake flow into a dedicated
virtual environment. This avoids relying on whichever `python3` happens to
be first on `PATH`, which may not be the one `kconfiglib` was installed for:

```
python3 -m venv $BAO_DEMOS_NUTTX_KCONFIG_VENV
$BAO_DEMOS_NUTTX_KCONFIG_BIN/pip install kconfiglib
```
---
